...
import ac.rino.rinoac.events.packets.CheckManagerListener;
import ac.rino.rinoac.manager.*;
import ac.rino.rinoac.predictionengine.MovementCheckRunner;
import ac.rino.rinoac.predictionengine.PointThreeEstimator;
import ac.rino.rinoac.predictionengine.UncertaintyHandler;
...
import ac.rino.rinoac.utils.data.packetentity.PacketEntitySelf;
import ac.rino.rinoac.utils.data.tags.SyncedTags;
import ac.rino.rinoac.utils.enums.FluidTag;
import ac.rino.rinoac.utils.enums.Pose;
import ac.rino.rinoac.utils.latency.*;
import ac.rino.rinoac.utils.math.RinoMath;
import ac.rino.rinoac.utils.math.TrigHandler;
...
public class RinoPlayer implements GrimUser {
    ...
    public void disconnect(Component reason) {
        ...
        if (bukkitPlayer != null) {
            FoliaScheduler.getEntityScheduler().execute(bukkitPlayer, RinoAPI.INSTANCE.getPlugin(), () -> {
    ...
    public void pollData() {
        ...
        if ((System.nanoTime() - getPlayerClockAtLeast()) > RinoAPI.INSTANCE.getConfigManager().getMaxPingTransaction() * 1e9) {
            timedOut();
        }

        if (!RinoAPI.INSTANCE.getPlayerDataManager().shouldCheck(user)) {
            RinoAPI.INSTANCE.getPlayerDataManager().remove(user);
        }
    ...
    public int getTransactionPing() {
        return RinoMath.floor(transactionPing / 1e6);
    }
