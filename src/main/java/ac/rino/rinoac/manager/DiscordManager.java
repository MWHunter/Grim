...
public class DiscordManager implements Initable {
    ...
    public void start() {
        try {
            if (!RinoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("enabled", false)) return;
            String webhook = RinoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("webhook", "");
            if (webhook.isEmpty()) {
            ...
            embedTitle = RinoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-title", "**Grim Alert**");

            try {
                embedColor = Color.decode(RinoAPI.INSTANCE.getConfigManager().getConfig().getStringElse("embed-color", "#00FFFF")).getRGB();
            } catch (NumberFormatException e) {
                LogUtil.warn("Discord embed color is invalid");
            }
            StringBuilder sb = new StringBuilder();
            for (String string : RinoAPI.INSTANCE.getConfigManager().getConfig().getStringListElse("violation-content", getDefaultContents())) {
    ...
    public void sendAlert(RinoPlayer player, String verbose, String checkName, String violations) {
        if (client != null) {

            String content = staticContent;
            content = content.replace("%check%", checkName);
            content = content.replace("%violations%", violations);
            content = RinoAPI.INSTANCE.getExternalAPI().replaceVariables(player, content, false);
