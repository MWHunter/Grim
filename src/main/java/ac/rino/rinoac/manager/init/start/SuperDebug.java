...
import ac.rino.rinoac.utils.lists.EvictingQueue;
...
public final class SuperDebug extends Check implements PostPredictionCheck {
    ...
    public SuperDebug(RinoPlayer player) {
        super(player);
    }
    ...
    public void onPredictionComplete(final PredictionComplete predictionComplete) {
        ...
        sb.append("Grim Version: ").append(RinoAPI.INSTANCE.getExternalAPI().getGrimVersion());
        sb.append("\n");
        ...
        for (int y = RinoMath.floor(player.boundingBox.minY) - 2; y <= RinoMath.ceil(player.boundingBox.maxY) + 2; y++) {
            for (int z = RinoMath.floor(player.boundingBox.minZ) - 2; z <= RinoMath.ceil(player.boundingBox.maxZ) + 2; z++) {
                maxPosLength = (int) Math.max(maxPosLength, Math.ceil(Math.log10(Math.abs(z))));
                for (int x = RinoMath.floor(player.boundingBox.minX) - 2; x <= RinoMath.ceil(player.boundingBox.maxX) + 2; x++) {
        ...
        for (int y = RinoMath.ceil(player.boundingBox.maxY) + 2; y >= RinoMath.floor(player.boundingBox.minY) - 2; y--) {
            sb.append("y: ");
            sb.append(y);
            sb.append("\n");
    ...
    private Vector getPlayerMathMovement(RinoPlayer player, Vector wantedMovement, float f2) {
