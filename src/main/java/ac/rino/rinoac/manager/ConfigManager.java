...
public class ConfigManager {
    ...
    @Getter
    private final File configFile = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "config.yml");
    @Getter
    private final File messagesFile = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "messages.yml");
    @Getter
    ...
    public ConfigManager() {
        ...
        // load config
        RinoAPI.INSTANCE.getPlugin().getDataFolder().mkdirs();
        config = new DynamicConfig();
        config.addSource(RinoAC.class, "config", getConfigFile());
        config.addSource(RinoAC.class, "messages", getMessagesFile());
        config.addSource(RinoAC.class, "discord", getDiscordFile());
        config.addSource(RinoAC.class, "punishments", getPunishFile());
    ...
    public void reload() {
        ...
        maxPingTransaction = (int) RinoMath.clamp(configuredMaxTransactionTime, 1, 180);
    ...
    private void upgrade() {
        File config = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "config.yml");
        if (config.exists()) {
            try {
    ...
    private void removeLegacyTwoPointOne(File config) throws IOException {
        ...
        Files.move(config.toPath(), new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "config-2.1.old.yml").toPath());
    }
    ...
    private void addMissingPunishments() {
        File config = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "punishments.yml");
        String configString;
        if (config.exists()) {
            try {
    ...
    private void fixBadPacketsAndAdjustPingConfig(File config, String configString) {
        ...
        File punishConfig = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "punishments.yml");
    ...
    private void newOffsetNewDiscordConf(File config, String configString) throws IOException {
        ...
        File discordFile = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "discord.yml");
    ...
    private void addSuperDebug(File config, String configString) throws IOException {
        ...
        File messageFile = new File(RinoAPI.INSTANCE.getPlugin().getDataFolder(), "messages.yml");
