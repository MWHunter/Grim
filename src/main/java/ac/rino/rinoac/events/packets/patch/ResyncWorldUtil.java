package ac.rino.rinoac.events.packets.patch;
...
public class ResyncWorldUtil {
    ...
    public static void resyncPosition(RinoPlayer player, Vector3i pos) {
        resyncPositions(player, pos.getX(), pos.getY(), pos.getZ(), pos.getX(), pos.getY(), pos.getZ());
    }

    public static void resyncPositions(RinoPlayer player, SimpleCollisionBox box) {
        resyncPositions(player, RinoMath.floor(box.minX), RinoMath.floor(box.minY), RinoMath.floor(box.minZ),
                RinoMath.ceil(box.maxX), RinoMath.ceil(box.maxY), RinoMath.ceil(box.maxZ));
    }

    public static void resyncPositions(RinoPlayer player, int minBlockX, int mY, int minBlockZ, int maxBlockX, int mxY, int maxBlockZ) {
        ...
        FoliaScheduler.getEntityScheduler().execute(player.bukkitPlayer, RinoAPI.INSTANCE.getPlugin(), () -> {
