...
import ac.rino.rinoac.checks.impl.badpackets.BadPacketsX;
import ac.rino.rinoac.checks.impl.badpackets.BadPacketsZ;
import ac.rino.rinoac.events.packets.patch.ResyncWorldUtil;
import ac.rino.rinoac.player.RinoPlayer;
import ac.rino.rinoac.utils.anticheat.update.*;
import ac.rino.rinoac.utils.blockplace.BlockPlaceResult;
import ac.rino.rinoac.utils.blockplace.ConsumesBlockPlace;
import ac.rino.rinoac.utils.collisions.HitboxData;
...
import ac.rino.rinoac.utils.data.*;
import ac.rino.rinoac.utils.inventory.Inventory;
import ac.rino.rinoac.utils.latency.CompensatedWorld;
...
public class CheckManagerListener extends PacketListenerAbstract {
    ...
    public static HitData traverseBlocks(RinoPlayer player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        // I guess go back by the collision epsilon?
        double endX = RinoMath.lerp(-1.0E-7D, end.x, start.x);
        double endY = RinoMath.lerp(-1.0E-7D, end.y, start.y);
        double endZ = RinoMath.lerp(-1.0E-7D, end.z, start.z);
        double startX = RinoMath.lerp(-1.0E-7D, start.x, end.x);
        double startY = RinoMath.lerp(-1.0E-7D, start.y, end.y);
        double startZ = RinoMath.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = RinoMath.floor(startX);
        int floorStartY = RinoMath.floor(startY);
        int floorStartZ = RinoMath.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - RinoMath.frac(startX) : RinoMath.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - RinoMath.frac(startY) : RinoMath.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - RinoMath.frac(startZ) : RinoMath.frac(startZ));
    ...
    private static void placeWaterLavaSnowBucket(RinoPlayer player, ItemStack held, StateType toPlace, InteractionHand hand) {
    ...
    public static void handleQueuedPlaces(RinoPlayer player, boolean hasLook, float pitch, float yaw, long now) {
    ...
    private static void handleUseItem(RinoPlayer player, ItemStack placedWith, InteractionHand hand) {
    ...
    private static void handleBlockPlaceOrUseItem(PacketWrapper packet, RinoPlayer player) {
    ...
    private boolean isMojangStupid(RinoPlayer player, WrapperPlayClientPlayerFlying flying) {
        ...
        if (!player.packetStateData.lastPacketWasTeleport && flying.hasPositionChanged() && flying.hasRotationChanged() &&
            ...
            if (PacketEvents.getAPI().getServerManager().getVersion().isOlderThanOrEquals(ServerVersion.V_1_9)) {
                if (RinoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("cancel-duplicate-packet", true)) {
            ...
            if (!RinoAPI.INSTANCE.getConfigManager().isIgnoreDuplicatePacketRotation()) {
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (WrapperPlayClientPlayerFlying.isFlying(event.getPacketType())) {
            ...
            boolean ignoreRotation = player.packetStateData.lastPacketWasOnePointSeventeenDuplicate && RinoAPI.INSTANCE.getConfigManager().isIgnoreDuplicatePacketRotation();
    ...
    private static void placeBucket(RinoPlayer player, InteractionHand hand) {
    ...
    public static void setPlayerItem(RinoPlayer player, InteractionHand hand, ItemType type) {
    ...
    private void handleFlying(RinoPlayer player, double x, double y, double z, float yaw, float pitch, boolean hasPosition, boolean hasLook, boolean onGround, TeleportAcceptData teleportData, PacketReceiveEvent event) {
    ...
    private static void placeLilypad(RinoPlayer player, InteractionHand hand) {
        HitData data = getNearestHitResult(player, null, true);
    ...
    private static HitData getNearestHitResult(RinoPlayer player, StateType heldItem, boolean sourcesHaveHitbox) {
