...
public class ProxyAlertMessenger extends PacketListenerAbstract {
    ...
    public ProxyAlertMessenger() {
        ...
        if (usingProxy) {
            ...
            RinoAPI.INSTANCE.getPlugin().getServer().getMessenger().registerOutgoingPluginChannel(RinoAPI.INSTANCE.getPlugin(), "BungeeCord");
        }
    }
    ...
    public static void sendPluginMessage(String message) {
        ...
        Iterables.getFirst(Bukkit.getOnlinePlayers(), null).sendPluginMessage(RinoAPI.INSTANCE.getPlugin(), "BungeeCord", out.toByteArray());
    }

    public static boolean canSendAlerts() {
        return usingProxy && RinoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.send", false) && Bukkit.getOnlinePlayers().size() > 0;
    }

    public static boolean canReceiveAlerts() {
        return usingProxy && RinoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("alerts.proxy.receive", false) && RinoAPI.INSTANCE.getAlertManager().getEnabledAlerts().size() > 0;
    }
