...
import ac.rino.rinoac.checks.impl.movement.NoSlowE;
...
import ac.rino.rinoac.utils.inventory.EnchantmentHelper;
...
public class PacketEntitySelf extends PacketEntity {
    ...
    public PacketEntitySelf(RinoPlayer player) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    }

    public PacketEntitySelf(RinoPlayer player, PacketEntitySelf old) {
        super(player, EntityTypes.PLAYER);
        this.player = player;
    ...
    @Override
    protected void initAttributes(RinoPlayer player) {
        ...
        trackAttribute(ValuedAttribute.ranged(Attributes.PLAYER_SNEAKING_SPEED, 0.3, 0, 1)
                .withGetRewriter(value -> {
                    ...
                    final double clamped = RinoMath.clampFloat(0.3F + (swiftSneak * 0.15F), 0f, 1f);
                    if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) {
                        return clamped;
                    }
    ...
    @Override
    public void onFirstTransaction(boolean relative, boolean hasPos, double relX, double relY, double relZ, RinoPlayer player) {
        // Player ignores this
    }
