...
import ac.rino.rinoac.utils.collisions.blocks.*;
import ac.rino.rinoac.utils.collisions.blocks.connecting.DynamicFence;
import ac.rino.rinoac.utils.collisions.blocks.connecting.DynamicPane;
import ac.rino.rinoac.utils.collisions.blocks.connecting.DynamicWall;
...
public enum CollisionData {
    ...
    DRIPSTONE((player, version, data, x, y, z) -> {
        ...
        return box.offset(RinoMath.clamp((((i & 15L) / 15.0F) - 0.5D) * 0.5D, -0.125f, 0.125f), 0, RinoMath.clamp((((i >> 8 & 15L) / 15.0F) - 0.5D) * 0.5D, -0.125f, 0.125f));
    ...
    public CollisionBox getMovementCollisionBox(RinoPlayer player, ClientVersion version, WrappedBlockState block, int x, int y, int z) {
        if (this.box != null)
    ...
    public CollisionBox getMovementCollisionBox(RinoPlayer player, ClientVersion version, WrappedBlockState block) {
        if (this.box != null)
