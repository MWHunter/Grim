...
import ac.rino.rinoac.utils.data.BlockPrediction;
import ac.rino.rinoac.utils.data.Pair;
import ac.rino.rinoac.utils.data.PistonData;
import ac.rino.rinoac.utils.data.ShulkerData;
import ac.rino.rinoac.utils.data.packetentity.PacketEntity;
import ac.rino.rinoac.utils.data.packetentity.PacketEntityShulker;
...
public class CompensatedWorld {
    ...
    public void stopPredicting(PacketWrapper<?> wrapper) {
        ...
        if (PacketEvents.getAPI().getServerManager().getVersion().isNewerThanOrEquals(ServerVersion.V_1_19)) {
            ...
            Bukkit.getScheduler().runTask(RinoAPI.INSTANCE.getPlugin(), () -> {
    ...
    public double getWaterFluidLevelAt(double x, double y, double z) {
        return getWaterFluidLevelAt(RinoMath.floor(x), RinoMath.floor(y), RinoMath.floor(z));
    }
