...
import ac.rino.rinoac.utils.chunks.Column;
...
import ac.rino.rinoac.utils.data.tags.SyncedTags;
...
public class Collisions {
    ...
    public static Vector collide(RinoPlayer player, double desiredX, double desiredY, double desiredZ) {
        return collide(player, desiredX, desiredY, desiredZ, desiredY, null);
    }

    public static Vector collide(RinoPlayer player, double desiredX, double desiredY, double desiredZ, double clientVelY, VectorData data) {
    ...
    public static boolean addWorldBorder(RinoPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
        ...
        if (player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_8)) {
            ...
            double minX = Math.floor(RinoMath.clamp(centerX - size, -absoluteMaxSize, absoluteMaxSize));
            double minZ = Math.floor(RinoMath.clamp(centerZ - size, -absoluteMaxSize, absoluteMaxSize));
            double maxX = Math.ceil(RinoMath.clamp(centerX + size, -absoluteMaxSize, absoluteMaxSize));
            double maxZ = Math.ceil(RinoMath.clamp(centerZ + size, -absoluteMaxSize, absoluteMaxSize));
    ...
    public static boolean getCollisionBoxes(RinoPlayer player, SimpleCollisionBox wantedBB, List<SimpleCollisionBox> listOfBlocks, boolean onlyCheckCollide) {
    ...
    public static boolean isEmpty(RinoPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static Vector maybeBackOffFromEdge(Vector vec3, RinoPlayer player, boolean overrideVersion) {
    ...
    public static boolean isAboveGround(RinoPlayer player) {
    ...
    public static void handleInsideBlocks(RinoPlayer player) {
    ...
    private static boolean isSlidingDown(Vector vector, RinoPlayer player, int locationX, int locationY,
                                         int locationZ) {
        if (player.onGround) {
            return false;
    ...
    // 0.03 hack
    public static boolean checkStuckSpeed(RinoPlayer player, double expand) {
        // Use the bounding box for after the player's movement is applied
    ...
    public static boolean suffocatesAt(RinoPlayer player, SimpleCollisionBox playerBB) {
    ...
    public static boolean doesBlockSuffocate(RinoPlayer player, int x, int y, int z) {
        WrappedBlockState data = player.compensatedWorld.getWrappedBlockStateAt(x, y, z);
    ...
    // Thanks Tuinity
    public static boolean hasMaterial(RinoPlayer player, SimpleCollisionBox checkBox, Predicate<Pair<WrappedBlockState, Vector3d>> searchingFor) {
    ...
    // Thanks Tuinity
    public static void forEachCollisionBox(RinoPlayer player, SimpleCollisionBox checkBox, Consumer<Vector3d> searchingFor) {
    ...
    public static boolean onClimbable(RinoPlayer player, double x, double y, double z) {
    ...
    public static boolean trapdoorUsableAsLadder(RinoPlayer player, double x, double y, double z, WrappedBlockState blockData) {
