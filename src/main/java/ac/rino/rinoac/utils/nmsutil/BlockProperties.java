...
import ac.rino.rinoac.utils.data.MainSupportingBlockData;
...
public class BlockProperties {
    public static float getFrictionInfluencedSpeed(float f, RinoPlayer player) {
     ...
     */
    public static StateType getOnPos(RinoPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static float getFriction(RinoPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_19_4)) {
    ...
    public static float getBlockSpeedFactor(RinoPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
    ...
    public static boolean onHoneyBlock(RinoPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
     ...
     */
    private static StateType getBlockPosBelowThatAffectsMyMovement(RinoPlayer player, MainSupportingBlockData mainSupportingBlockData, Vector3d playerPos) {
        Vector3i pos = getOnPos(player, playerPos, mainSupportingBlockData, 0.500001F);
        return player.compensatedWorld.getStateTypeAt(pos.x, pos.y, pos.z);
    }

    private static Vector3i getOnPos(RinoPlayer player, Vector3d playerPos, MainSupportingBlockData mainSupportingBlockData, float searchBelowPlayer) {
        ...
        if (mainBlockPos != null) {
            ...
            return shouldReturn ? mainBlockPos.withY(RinoMath.floor(playerPos.getY() - (double) searchBelowPlayer)) : mainBlockPos;
        } else {
            return new Vector3i(RinoMath.floor(playerPos.getX()), RinoMath.floor(playerPos.getY() - searchBelowPlayer), RinoMath.floor(playerPos.getZ()));
        }
    }

    public static float getMaterialFriction(RinoPlayer player, StateType material) {
        float friction = 0.6f;
    ...
    private static StateType getOnBlock(RinoPlayer player, double x, double y, double z) {
        StateType block1 = player.compensatedWorld.getStateTypeAt(RinoMath.floor(x), RinoMath.floor(y - 0.2F), RinoMath.floor(z));

        if (block1.isAir()) {
            StateType block2 = player.compensatedWorld.getStateTypeAt(RinoMath.floor(x), RinoMath.floor(y - 1.2F), RinoMath.floor(z));
    ...
    private static float getBlockSpeedFactorLegacy(RinoPlayer player, Vector3d pos) {
    ...
    private static float getBlockSpeedFactor(RinoPlayer player, StateType type) {
    ...
    private static float getModernVelocityMultiplier(RinoPlayer player, float blockSpeedFactor) {
        if (player.getClientVersion().isOlderThan(ClientVersion.V_1_21)) return blockSpeedFactor;
        return (float) RinoMath.lerp((float) player.compensatedEntities.getSelf().getAttributeValue(Attributes.GENERIC_MOVEMENT_EFFICIENCY), blockSpeedFactor, 1.0F);
    }
}
