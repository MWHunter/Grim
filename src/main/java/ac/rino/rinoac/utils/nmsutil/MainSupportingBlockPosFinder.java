...
public class MainSupportingBlockPosFinder {
    public MainSupportingBlockData findMainSupportingBlockPos(RinoPlayer player, MainSupportingBlockData lastSupportingBlock, Vector3d lastMovement, SimpleCollisionBox maxPose, boolean isOnGround) {
        if (!isOnGround) {
    ...
    private Optional<Vector3i> findSupportingBlock(RinoPlayer player, SimpleCollisionBox searchBox) {
