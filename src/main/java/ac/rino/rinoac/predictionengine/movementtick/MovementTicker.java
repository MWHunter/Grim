...
import ac.rino.rinoac.predictionengine.PlayerBaseTick;
...
public class MovementTicker {
    ...
    public MovementTicker(RinoPlayer player) {
        this.player = player;
    }

    public static void handleEntityCollisions(RinoPlayer player) {
    ...
    public void livingEntityTravel() {
        ...
        double lavaLevel = 0;
        if (canStandOnLava())
            lavaLevel = player.compensatedWorld.getLavaFluidLevelAt(RinoMath.floor(player.lastX), RinoMath.floor(player.lastY), RinoMath.floor(player.lastZ));
