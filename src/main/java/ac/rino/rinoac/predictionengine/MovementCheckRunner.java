package ac.rino.rinoac.predictionengine;

import ac.rino.rinoac.RinoAPI;
import ac.rino.rinoac.checks.Check;
import ac.rino.rinoac.checks.impl.movement.EntityControl;
import ac.rino.rinoac.checks.impl.prediction.Phase;
import ac.rino.rinoac.checks.type.PositionCheck;
import ac.rino.rinoac.manager.SetbackTeleportUtil;
import ac.rino.rinoac.player.RinoPlayer;
import ac.rino.rinoac.predictionengine.movementtick.MovementTickerHorse;
import ac.rino.rinoac.predictionengine.movementtick.MovementTickerPig;
import ac.rino.rinoac.predictionengine.movementtick.MovementTickerPlayer;
import ac.rino.rinoac.predictionengine.movementtick.MovementTickerStrider;
import ac.rino.rinoac.predictionengine.predictions.PredictionEngineNormal;
import ac.rino.rinoac.predictionengine.predictions.rideable.BoatPredictionEngine;
...
import ac.rino.rinoac.utils.data.packetentity.PacketEntityTrackXRot;
import ac.rino.rinoac.utils.enums.Pose;
...
public class MovementCheckRunner extends Check implements PositionCheck {
    ...
    public MovementCheckRunner(RinoPlayer player) {
        super(player);
    }
    ...
    private void check(PositionUpdate update) {
        ...
        if (!player.isFlying && player.isSneaking && Collisions.isAboveGround(player)) {
            ...
            double posX = Math.max(0.05, RinoMath.clamp(player.actualMovement.getX(), -16, 16) + 0.05);
            double posZ = Math.max(0.05, RinoMath.clamp(player.actualMovement.getZ(), -16, 16) + 0.05);
            double negX = Math.min(-0.05, RinoMath.clamp(player.actualMovement.getX(), -16, 16) - 0.05);
            double negZ = Math.min(-0.05, RinoMath.clamp(player.actualMovement.getZ(), -16, 16) - 0.05);
        ...
        if (player.bukkitPlayer != null && player.isGliding && player.predictedVelocity.isJump() && player.isSprinting
                && !RinoAPI.INSTANCE.getConfigManager().getConfig().getBooleanElse("exploit.allow-sprint-jumping-when-using-elytra", true)) {
