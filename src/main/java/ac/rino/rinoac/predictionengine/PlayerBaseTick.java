...
import ac.rino.rinoac.checks.impl.movement.NoSlowC;
import ac.rino.rinoac.player.RinoPlayer;
import ac.rino.rinoac.utils.collisions.datatypes.SimpleCollisionBox;
import ac.rino.rinoac.utils.data.attribute.ValuedAttribute;
...
import ac.rino.rinoac.utils.latency.CompensatedEntities;
...
public class PlayerBaseTick {
    ...
    public PlayerBaseTick(RinoPlayer player) {
        this.player = player;
    }

    public static boolean canEnterPose(RinoPlayer player, Pose pose, double x, double y, double z) {
        return Collisions.isEmpty(player, getBoundingBoxForPose(player, pose, x, y, z).expand(-1.0E-7D));
    }

    protected static SimpleCollisionBox getBoundingBoxForPose(RinoPlayer player, Pose pose, double x, double y, double z) {
    ...
    private void moveTowardsClosestSpaceLegacy(double x, double z) {
        int floorX = RinoMath.floor(x);
        int floorZ = RinoMath.floor(z);
        int floorY = RinoMath.floor(player.lastY + 0.5);
    ...
    public boolean updateFluidHeightAndDoFluidPushingLegacy(FluidTag tag, double multiplier) {
        ...
        int floorX = RinoMath.floor(aABB.minX);
        int ceilX = RinoMath.ceil(aABB.maxX);
        int floorY = RinoMath.floor(aABB.minY);
        int ceilY = RinoMath.ceil(aABB.maxY);
        int floorZ = RinoMath.floor(aABB.minZ);
        int ceilZ = RinoMath.ceil(aABB.maxZ);
